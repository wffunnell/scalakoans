...
class AboutClasses extends KoanSuite {
  ...
  koan("val parameters in class definition define getter") {
    val aClass = new ClassWithValParameter("valParam")
    aClass.name should be("valParam")
  }

  koan("var parameters in class definition define getter and setter") {
    val aClass = new ClassWithVarParameter("varParam")
    aClass.description should be("varParam")
